...
Translation tooling for the Paradym wallet is built on [Lingui](https://lingui.dev). Catalogs live under `apps/wallet/src/locales/<locale>/messages.json`. The `en` locale is the source of truth; all other locales must stay in sync with it.
...
3. Run `pnpm translations:ai:finalize` from the workspace root. This merges each `missing.json` back into `messages.json`, deletes the `missing.json` files, re-runs `translations:extract` + `translations:compile` in `apps/wallet`, and runs `style:fix` at the workspace root.
