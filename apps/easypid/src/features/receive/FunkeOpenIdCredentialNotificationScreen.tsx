...
export function FunkeCredentialNotificationScreen() {
  ...
  const onPresentationAccept = useCallback(
    async ({ pin, onPinComplete, onPinError }: onPinSubmitProps = {}) => {
      ...
      try {
        const credential = await paradym.openid4vc.acquireCredentialsAuthPresentationDuringIssuance({
          authorization,
  ...
  const acquireCredentialsAuth = useCallback(
    async (authorizationCode: string) => {
      if (!resolvedOffer || resolvedOffer.flow === 'pre-auth') {
      ...
      try {
        const credential = await paradym.openid4vc.acquireCredentialsAuth({
          resolvedCredentialOffer: resolvedOffer.resolvedCredentialOffer,
          resolvedAuthorizationRequest: resolvedOffer.resolvedAuthorizationRequest,
          authorizationCode,
          authorization,
  ...
  const acquireCredentialsPreAuth = useCallback(
    async (txCode?: string) => {
      if (resolvedOffer?.flow !== 'pre-auth') {
        setErrorReason(t(commonMessages.credentialInformationCouldNotBeExtracted))
        return
      }

      // Credentials will be acquired when the txCode is entered on the next screen
      if (resolvedOffer.txCodeInfo && !txCode) return

      try {
        const credential = await paradym.openid4vc.acquireCredentialsPreAuth({
          resolvedCredentialOffer: resolvedOffer.resolvedCredentialOffer,
    ...
    <SlideWizard
      steps={[
        ...
        },
        {
          ...
          screen: (
            <VerifyPartySlide
              ...
              onContinue={resolvedOffer.flow === 'pre-auth' ? acquireCredentialsPreAuth : undefined}
            />
          ),
        },
          ...
          : resolvedOffer.flow === 'auth-presentation-during-issuance'
            ...
            : {
                ...
                screen: (
                  <CredentialCardSlide
                    key="credential-card"
                    type={resolvedOffer.txCodeInfo ? 'pin' : 'noAuth'}
                    display={resolvedOffer.credentialDisplay}
                  />
                ),
              },
        ...
        resolvedOffer.flow === 'auth-presentation-during-issuance' && shouldUsePinForPresentation
          ...
          : undefined,
        resolvedOffer.flow === 'pre-auth' && resolvedOffer.txCodeInfo
          ? {
              step: 'tx-code',
              progress: 66,
              backIsCancel: true,
