...
export interface ReceivePidUseCaseBPrimeOptions
  extends Pick<
    ...
    | "onAttachCard"
    | "onRequestAccessRights"
    | "onStatusProgress"
    | "onCardAttachedChanged"
  > {
  agent: AppAgent;
...
export type ReceivePidUseCaseState =
  | "id-card-auth"
...
export class ReceivePidUseCaseBPrimeFlow {
  ...
  private errorCallbacks: AusweisAuthFlowOptions["onError"][] = [
    this.handleError,
  ];
  ...
  public authenticateUsingIdCard() {
    ...
    if (this.currentState !== "id-card-auth") {
      throw new Error(
        `Current state is ${this.currentState}. Expected id-card-auth`
      );
    }
    ...
    const authenticationPromise = new Promise((resolve, reject) => {
      const successCallback: AusweisAuthFlowOptions["onSuccess"] = ({
        ...
        this.successCallbacks = this.successCallbacks.filter(
          (c) => c === successCallback
        );
        resolve(null);
      };
  ...
  public async retrieveCredential() {
    try {
      ...
      if (!this.accessToken) {
        throw new Error(
          "Expected accessToken be defined in state retrieve-credential"
        );
      }

      const credentialConfigurationIdToRequest =
        this.resolvedCredentialOffer.offeredCredentials[0].id;
  ...
  public async acquireAccessToken() {
    ...
    try {
      if (!this.refreshUrl) {
        throw new Error(
          "Expected refreshUrl be defined in state acquire-access-token"
        );
      }
  ...
  private assertState({
    ...
    if (this.currentState !== expectedState) {
      throw new Error(
        `Expected state to be ${expectedState}. Found ${this.currentState}`
      );
    }
