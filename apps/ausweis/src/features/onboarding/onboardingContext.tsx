...
const onboardingStepsCFlow = [
  ...
  },
  {
    ...
    page: {
      ...
      subtitle:
        "To setup your digital identity we'll follow the following steps:",
    },
...
export function OnboardingContextProvider({
  ...
  const [receivePidUseCase, setReceivePidUseCase] =
    useState<ReceivePidUseCase>();
  const [receivePidUseCaseState, setReceivePidUseCaseState] = useState<
    ReceivePidUseCaseState | "initializing"
  >();
  ...
  const currentStep = onboardingStepsCFlow.find(
    (step) => step.step === currentStepName
  );
  if (!currentStep) throw new Error(`Invalid step ${currentStepName}`);

  const goToNextStep = useCallback(() => {
    const currentStepIndex = onboardingStepsCFlow.findIndex(
      (step) => step.step === currentStepName
    );
  ...
  const selectFlow = (flow: "c" | "bprime") => {
    console.log(`toggled to flow: ${flow}`);
  ...
  const [onIdCardPinReEnter, setOnIdCardPinReEnter] =
    useState<(idCardPin: string) => Promise<void>>();
  ...
  const reset = ({
    ...
    const stepsToCompleteAfterReset = onboardingStepsCFlow
      .slice(
        onboardingStepsCFlow.findIndex((step) => step.step === resetToStep)
      )
      .map((step) => step.step);
        ...
        try {
          ...
          setUserName(
            `${capitalizeFirstLetter(
              parsed.prettyClaims.given_name.toLowerCase()
            )} ${capitalizeFirstLetter(
  ...
  if (currentStep.step === "welcome") {
    ...
    screen = (
      <currentStep.Screen
        goToNextStep={onIdCardPinReEnter ?? onIdCardPinEnter}
      />
    );
    ...
    screen = (
      <currentStep.Screen goToNextStep={goToNextStep} userName={userName} />
    );
  } else {
...
export function useOnboardingContext() {
  ...
  if (!value) {
    throw new Error(
      "useOnboardingContext must be wrapped in a <OnboardingContext.Provider />"
    );
  }
