import { mdlSchemes, pidSchemes } from '@app/constants'
import { getMdlCode, type MdlAttributes } from '@app/utils/mdlCustomMetadata'
import {
  type ArfPidSdJwtVcAttributes,
  formatArfPidPlaceOfBirth,
  type PidMdocAttributes,
} from '@app/utils/pidCustomMetadata'
...
function CustomCredentialAttributesInner({ credential, scrollRef }: CustomCredentialAttributesProps) {
  if ([...pidSchemes.sdJwtVcVcts, ...pidSchemes.msoMdocDoctypes].includes(credential.metadata.type)) {
    return <ArfPidCredentialAttributes credential={credential} />
  }
  ...
  if (mdlSchemes.mdlMdocDoctypes.includes(credential.metadata.type)) {
    return <MdlCredentialAttributes credential={credential} />
  }
...
export function ArfPidCredentialAttributes({ credential }: CustomCredentialAttributesProps) {
  const { t } = useLingui()
...
export function MdlCredentialAttributes({ credential }: CustomCredentialAttributesProps) {
  const { t } = useLingui()
  const raw = (
    credential.rawAttributes as {
