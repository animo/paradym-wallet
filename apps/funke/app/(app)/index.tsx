...
} from "../../use-cases/ReceivePidUseCaseCFlow";
...
export default function Screen() {
  ...
  const [receivePidUseCaseCFlow, setReceivePidUseCaseCFlow] =
    useState<ReceivePidUseCaseCFlow>();
  const [receivePidUseCaseBPrimeFlow, setReceivePidUseCaseBPrimeFlow] =
    useState<ReceivePidUseCaseBPrimeFlow>();
  const [state, setState] = useState<
    ReceivePidUseCaseState | "not-initialized"
  >("not-initialized");
  const [credential, setCredential] = useState<string>();
  const { agent } = useAppAgent();

  useEffect(() => {
    ReceivePidUseCaseCFlow.initialize({ agent, onStateChange: setState }).then(
      (pidUseCase) => {
  ...
  const nextStep = async () => {
    ...
    if (flow === "b'") {
      ...
      if (state === "retrieve-credential") {
        const credential =
          await receivePidUseCaseBPrimeFlow.retrieveCredential();
        setCredential(credential.compactSdJwtVc);
      }
    }
  };
  ...
  return (
    <>
      ...
      <YStack>
        ...
        <Button.Solid
          onPress={() => (flow === "c" ? setFlow("b'") : setFlow("c"))}
        >
          toggle flow
        </Button.Solid>
        ...
        <Button.Solid
          disabled={state !== "retrieve-credential"}
          onPress={nextStep}
        >
          Retrieve credential
        </Button.Solid>
