...
import { parseMessageType, supportsIncomingMessageType } from '@credo-ts/didcomm/build/util/messageType'
import {
...
export const receiveCredentialFromOpenId4VciOffer = async ({
  ...
  const creds = credentials.map(({ credentials, ...credentialResponse }) => {
