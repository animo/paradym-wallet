import {
  ...
  ParadymWalletInvitationRetrievalError,
} from '../error'
import type { ParseInvitationResult, ParsedInvitationTypeData } from './parser'

const errorResponse = (error: ParadymWalletInvitationError, message: string) => {
  return {
    success: false,
    error,
    message,
...
export async function fetchInvitationDataUrl(dataUrl: string): Promise<FetchInvitationResult> {
  ...
  try {
    ...
    if (!response.ok) {
      return errorResponse(new ParadymWalletInvitationRetrievalError(), 'Unable to retrieve invitation.')
    }
...
function handleJsonResponse(json: unknown): FetchInvitationResult {
  ...
  if (!json || typeof json !== 'object' || Array.isArray(json)) {
    return errorResponse(new ParadymWalletInvitationNotRecognizedError(), 'Invitation not recognized.')
  }
