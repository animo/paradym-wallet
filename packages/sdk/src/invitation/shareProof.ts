        ...
        await Promise.all(
          Object.entries(
            Object.keys(selectedCredentials).length > 0
              ...
              : paradym.agent.modules.openid4vc.holder.selectCredentialsForDcqlRequest(resolvedRequest.queryResult)
            ...
            const updatedCredentials = await Promise.all(
              credentials.map(async (credential) => ({
                ...credential,
                credentialRecord:
                  credential.credentialRecord instanceof W3cV2CredentialRecord
                    ? credential.credentialRecord
                    : await handleBatchCredential(paradym, credential.credentialRecord),
              }))
            )
