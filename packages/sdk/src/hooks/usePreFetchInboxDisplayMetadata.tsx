...
export const usePreFetchInboxDisplayMetadata = ({ paradym }: { paradym: ParadymWalletSdk }) => {
    ...
    credentialExchangeRecords.map(async (record) => {
      ...
      const outOfBandRecord =
        !connection && record.parentThreadId
          ? await paradym.agent.modules.outOfBand.findByReceivedInvitationId(record.parentThreadId)
          : undefined

      const formatData = await paradym.agent.modules.credentials.getFormatData(record.id)
      ...
      if (issuerName || schemaName) {
        ...
        await paradym.agent.modules.credentials.update(record)
      }
    })
  ...
  useEffect(() => {
    proofExchangeRecords.map(async (record) => {
      ...
      const formatData = await paradym.agent.modules.proofs.getFormatData(record.id)
      ...
      if (verifierName || proofName) {
        ...
        await paradym.agent.modules.proofs.update(record)
      }
    })
