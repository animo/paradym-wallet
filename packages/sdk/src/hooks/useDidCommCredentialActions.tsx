...
import { type FormattedAttributeString, mapAttributeName } from '../format/attributes'
import {
...
function useOfferAttributes(credentialExchangeId: string) {
  ...
  const { data, status } = useQuery({
    ...
    queryFn: async () => {
      const formatData = await paradym.agent.didcomm.credentials.getFormatData(credentialExchangeId)
...
export function useDidCommCredentialActions(credentialExchangeId: string) {
  ...
  const { mutateAsync: acceptCredentialMutation, status: acceptStatus } = useMutation({
    ...
    mutationFn: async () => {
      ...
      await paradym.agent.didcomm.credentials.acceptOffer({ credentialExchangeRecordId: credentialExchangeId })
  ...
  const { mutateAsync: declineCredentialMutation, status: declineStatus } = useMutation({
    ...
    mutationFn: async () => {
      ...
      await paradym.agent.didcomm.credentials.declineOffer({
