...
type _AcceptPresentationOptions = {
  selectedCredentials?: { [inputDescriptorId: string]: string }
  storeAsActivity?: boolean
}

type _DeclinePresentationOptions = {
...
export function useDidCommPresentationActions(proofExchangeId: string) {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const formatData = await paradym.agent.didcomm.proofs.getFormatData(proofExchangeId)

      const proofRequest = formatData.request?.anoncreds ?? formatData.request?.indy

      const credentialsForRequest = await paradym.agent.didcomm.proofs.getCredentialsForRequest({
      ...
      const requestMessage = await paradym.agent.didcomm.proofs.findRequestMessage(proofExchangeId)
      const purpose =
  ...
  const { mutateAsync: acceptMutateAsync, status: acceptStatus } = useMutation({
    ...
    mutationFn: async (selectedCredentials?: { [inputDescriptorId: string]: string }) => {
      ...
      await paradym.agent.didcomm.proofs.acceptRequest({
        proofExchangeRecordId: proofExchangeId,
  ...
  const { mutateAsync: declineMutateAsync, status: declineStatus } = useMutation({
    ...
    mutationFn: async () => {
      ...
      await paradym.agent.didcomm.proofs.declineRequest({
        proofExchangeRecordId: proofExchangeId,
