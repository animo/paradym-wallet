...
import { YStack, Heading, Button, ScrollView, Paragraph, Sheet, Stack, XStack } from '@internal/ui'
import { BottomSheetScrollView, RefreshCw } from '@internal/ui/src'
...
interface PresentationNotificationScreenProps {
  ...
  onDecline: () => void
  verifierName?: string
  selectedCredentials: { [inputDescriptorId: string]: number | undefined }
            ...
            {currentSubmissionEntry?.credentials.map((c, credentialIndex) => {
              return (
                <CredentialRowCard
                  onPress={() => {
                    onSelectCredentialForInputDescriptor(
                      currentSubmissionEntry.inputDescriptorId,
                      credentialIndex
                    )
