		...
		modules: {
			askar: new AskarModule({
				...
				store: {
					...
					key: walletKey,
					keyDerivationMethod:
						keyDerivation === "raw" ? "raw" : "kdf:argon2i:mod",
				},
			}),
			...
			x509: new X509Module({
				getTrustedCertificatesForVerification: (
					...
					if (verification.type === "credential") {
						...
						return [
							certificateChain[certificateChain.length - 1].toString("pem"),
						];
					}
				...
				trustedCertificates:
					trustedX509Certificates.length > 0
...
export type ParadymAppAgent = Awaited<
	ReturnType<typeof initializeParadymAgent>
>;
export type EasyPIDAppAgent = Awaited<
	ReturnType<typeof initializeEasyPIDAgent>
>;
export type EitherAgent = ParadymAppAgent | EasyPIDAppAgent;

export const isParadymAgent = (
	agent: EitherAgent,
): agent is ParadymAppAgent => {
	return "anoncreds" in agent.modules;
};

export const isEasyPIDAgent = (
	agent: EitherAgent,
): agent is EasyPIDAppAgent => {
	return !("anoncreds" in agent.modules);
};
