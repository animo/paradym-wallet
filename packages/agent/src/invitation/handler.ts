...
export const receiveCredentialFromOpenId4VciOffer = async ({
  ...
  const resolvedCredentialOffer =
    await agent.modules.openId4VcHolder.resolveCredentialOffer(offerUri);
    ...
    credentialBindingResolver: async ({
      ...
      try {
        ...
        agent.config.logger.warn(
          "Could not create a key in the secure element",
  ...
  if (!firstCredential)
    throw new Error("Error retrieving credential using pre authorized flow.");
  ...
  const openId4VcMetadata = extractOpenId4VcCredentialMetadata(
    resolvedCredentialOffer
      .offeredCredentials[0] as OpenId4VciCredentialSupportedWithId,
    {
      display:
        resolvedCredentialOffer.metadata.credentialIssuerMetadata.display,
...
export const shareProof = async ({
  ...
  const credentials = Object.fromEntries(
    credentialsForRequest.requirements.flatMap((requirement) =>
      requirement.submissionEntry.map((entry) => {
        ...
        const credential =
          entry.verifiableCredentials.find(
            (vc) => vc.credentialRecord.id === credentialId
  ...
  if (
    result.serverResponse.status < 200 ||
    result.serverResponse.status > 299
  ) {
    throw new Error(
      `Error while accepting authorization request. ${
        result.serverResponse.body as string
      }`
    );
  }
...
> {
  ...
  if (requestMessages.length === 0) {
    if (
      ...
      invitation.handshakeProtocols.length === 0
    ) {
      agent.config.logger.error(
        "No requests and no handshake protocols found in invitation."
      );
      return {
        success: false,
  ...
  // Validate the type of the request message
    ...
    const isValidRequestMessage =
      supportsIncomingMessageType(
        parsedMessageType,
        V1OfferCredentialMessage.type
      ) ||
      supportsIncomingMessageType(
        parsedMessageType,
        V2OfferCredentialMessage.type
      ) ||
      supportsIncomingMessageType(
        parsedMessageType,
        V1RequestPresentationMessage.type
      ) ||
      supportsIncomingMessageType(
        parsedMessageType,
        V2RequestPresentationMessage.type
      );

    if (!isValidRequestMessage) {
      agent.config.logger.error(
        "Message request is not from supported protocol."
      );
      return {
        success: false,
        error: "Invalid invitation.",
  ...
  const credentialOffer = agent.events
    ...
    .pipe(
      ...
      filter(
        (event) => event.payload.credentialRecord.connectionId === connectionId
      )
    );
  ...
  try {
    ...
    const receiveInvitationResult = await agent.oob.receiveInvitation(
      invitation,
      {
    ...
    agent.config.logger.error(
      `Error while receiving invitation: ${error as string}`
    );
  ...
  try {
    ...
    const outOfBandRepository =
      agent.dependencyManager.resolve(OutOfBandRepository);
